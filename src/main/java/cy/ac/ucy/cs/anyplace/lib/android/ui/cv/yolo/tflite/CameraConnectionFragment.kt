...
class CameraConnectionFragment private constructor(
  ...
  companion object {
    ...
    fun chooseOptimalSize(choices: Array<Size?>, width: Int, height: Int): Size {
      ...
      LOG.V3(TAG, "Desired size: $desiredSize, min size: ${minSize}x${minSize}")
      LOG.V3(TAG, "Valid preview sizes: [${TextUtils.join(", ", bigEnough)}]")
      LOG.V3(TAG, "Rejected preview sizes: [${TextUtils.join(", ", tooSmall)}]")
      if (exactSizeFound) {
        LOG.V3(TAG, "Exact size match found.")
        return desiredSize
      }
      ...
      return if (bigEnough.size > 0) {
        // TODO: use KT
        val chosenSize = Collections.min(bigEnough, CompareSizesByArea() as Comparator<Size?>)
        LOG.V3(TAG, "Chosen size: " + chosenSize!!.width + "x" + chosenSize.height)
        chosenSize
      } else {
  ...
  private fun setUpCameraOutputs() {
    ...
    try {
      ...
      if (orientation == Configuration.ORIENTATION_LANDSCAPE) {
        textureView!!.setAspectRatio(sz.width, sz.height)
      } else {
        textureView!!.setAspectRatio(sz.height, sz.width)
      }
    } catch (e: CameraAccessException) {
      LOG.E(TAG, e)
    } catch (e: NullPointerException) {
      // Currently an NPE is thrown when the Camera2API is used but not supported on the
      showToast(getString(R.string.tfe_od_camera_error))
  ...
  private fun createCameraPreviewSession() {
    try {
      ...
      LOG.V2(TAG, "Opening camera preview: " + previewSize!!.width + "x" + previewSize!!.height)
  ...
  private fun configureTransform(viewWidth: Int, viewHeight: Int) {
    ...
    val bufferRect = RectF(0f, 0f, sz.height.toFloat(), sz.width.toFloat())
    val centerX = viewRect.centerX()
    val centerY = viewRect.centerY()
    if (Surface.ROTATION_90 == rotation || Surface.ROTATION_270 == rotation) {
      bufferRect.offset(centerX - bufferRect.centerX(), centerY - bufferRect.centerY())
      matrix.setRectToRect(viewRect, bufferRect, Matrix.ScaleToFit.FILL)
      val scale = max(viewHeight.toFloat() / sz.height, viewWidth.toFloat() / sz.width
      )
  ...
  /** Callback to init Activity data once the selected preview size is known. */
