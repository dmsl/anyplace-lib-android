...
open class MapOverlays(
  ...
  fun removeHeatmap() { scope.launch(Dispatchers.Main) { heatmapOverlay?.remove() } }
  /**
   * Loads heatmap on map, when a level was loaded
   * - right after loading the levelplan
   */
  suspend fun loadHeatmapOnLevel() {
    val MT = ::loadHeatmapOnLevel.name
    LOG.E(TG, MT)
    ...
    // levelplan loaded. trigger a heatmap render
    // (it is not done directly (through [renderHeatmap]).
    // instead it is done through a [Flow] ([VM.reloadHeatmap])
    VM.reloadHeatmap.update { true }
  }

  /**
   * Gets the weighted locations and renders the heatmap
   * - heatmap is only render on logger
   * - navigator is for navigation. this functionality concerns only the logger
   */
  suspend fun renderHeatmap() {
    val gmap = VM.ui.map.obj
    val MT = ::renderHeatmap.name
    if (!app.isLogger()) return

    removeHeatmap()
    if (!app.dsCvMap.read.first().cvLoggerHeatmaps) return
  ...
  suspend fun getWeightedLocationList() : List<WeightedLatLng> {
    ...
    LOG.D(TG, "$MT: points in ${points.size} locations")
    points.forEach {
      try {
